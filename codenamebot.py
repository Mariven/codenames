...
class Game:
        ...
        def removeplayer(self, nick):
            ...
            if nick not in self.players:
                PRIVMSG(owner, nick + ' is not playing.')
                return
...
while True:
    ...
    if thing=='NICK':
        ...
        if echo:
            PRIVMSG(owner, (timestamp+chan+' <'+author+'> '+''.join([x+' ' for x in data.split(' ')[3:]])[1:].split('\r')[0]).replace('ACTION','*'))
